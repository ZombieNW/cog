...
export const handleSettingsCommand = async (message:string): Promise<string> => {
    ...
    if (parts[0] === 'provider' && parts.length === 2) {
        if (!parts[1]) return CLI_COLORS.ERROR + "Missing provider name" + CLI_COLORS.RESET;
    ...
    if (parts.length >= 3) {
        ...
        if (!provider) return CLI_COLORS.ERROR + "Missing provider name" + CLI_COLORS.RESET;
        const key = parts[1];
    ...
    return CLI_COLORS.ERROR + "Invalid settings command. Usage:\n" +
           "  settings                           - Show current settings\n" +
           "  settings llm <provider>            - Switch LLM provider\n" +
           "  settings <provider> <key> <value>  - Set provider setting" + CLI_COLORS.RESET;
}
...
const switchProvider = (provider: string): string => {
    ...
    if (!config.providers[provider]) {
        return CLI_COLORS.ERROR + `Unknown LLM provider: ${provider}\n` +
               `Available providers: ${Object.keys(config.providers).join(', ')}` + CLI_COLORS.RESET;
    }
...
const setProviderSetting = (provider: string, key: string, value: string): string => {
    const config = loadConfig();
    
    if (!config.providers[provider]) {
        return CLI_COLORS.ERROR + `Unknown LLM provider: ${provider}` + CLI_COLORS.RESET;
    }
    ...
    const displayValue = key.toLowerCase().includes('key') ? '******' : value;
    return CLI_COLORS.SUCCESS + `Set ${provider}.${key} = ${displayValue}` + CLI_COLORS.RESET;
};